...
**Two of the eight "missing protections" it had reported were fabrications of the truncation** — the leases do state where the deposit is held and the conditions for withholding it, in text the single prompt never reached. A tool telling a renter their lease is missing a legally required term it actually contains is the same class of error as a false red flag, and it was invisible because nothing crashed. The results file now records `protection_windows` per document so a single-window assumption cannot creep back in unnoticed. Re-run cost: **$0.0223**, and it also produced the first protections measurement for the two long agreements, which the old clause cap had refused outright.
