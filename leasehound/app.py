...
# The stylesheet lives in style.css, next to this file, and is read at import.
#
# It was 501 lines of a 1,742-line module, which made app.py look like the largest thing
# in the project when its actual logic is 599 lines. Splitting it costs one read and
# buys syntax highlighting, a diff that says "css" when only the styling changed, and a
# file an editor will check for unbalanced braces. Nothing about how it is applied
# changes: gradio still receives it as a string through UI_STYLE.
#
# Read, not imported as a package resource: the app is deployed by copying the source
# tree (see the Dockerfile), which is also how leasehound/assets/*.svg has always been
# found. package-data in pyproject.toml covers the wheel case.
