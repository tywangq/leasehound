...
| scan-mode retrieval, 79 labelled clauses | governing section arrives 79/79 — [all 40 partial misses were one section; the fix cost precision in July and, re-gated in August, no longer does](evaluation/README.md#the-labelled-set-said-the-opposite-and-the-gate-agreed-with-it) |
| permissive vs prohibited, 32 labelled clauses | the shipped index reads two of the ten prohibitions as **green with no citation**; [the rejected fix flags 10/10 and produces 0 false reds](evaluation/README.md#the-labelled-set-said-the-opposite-and-the-gate-agreed-with-it) |
...
| [Permissive vs prohibited](evaluation/README.md#the-labelled-set-said-the-opposite-and-the-gate-agreed-with-it) | Can the judge tell "you may" from "you must only"? | the labelled set **reversed** that rejection — and found the shipped index reading two prohibitions as green |
...
- **Decide whether to ship the enumerated split, and run the silver set first** — the blocker named here for a week is gone. The [labelled permissive-vs-mandatory set](evaluation/README.md#the-labelled-set-said-the-opposite-and-the-gate-agreed-with-it) got built rather than skipped, and it reversed the finding: the split flags 10/10 prohibited clauses to the shipped index's 8/10 and produces **0 false reds to its 2**, and the re-run gold gate now passes at precision 1.000. Two things stand between that and shipping. The 40-lease silver set (~$0.6) is where precision is a measured lower bound rather than a perfect 1.000, and it has never seen the split. And enabling it changes what `export_runtime_db.py` exports and what the image carries, so it is a deployment change, not a flag flip. Worth noting what the same set found about the *shipped* index: it reads the exculpation and arbitration-cost prohibitions as green with no citation at all, which 18/18 recall never covered because no labelled lease plants a clause of either shape.
