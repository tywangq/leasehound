...
|  | cost & latency | 190 logged scans (9–15 clauses) | **≈ $0.011/scan** · p50 9.3 s · [p95 15.8 s is the provider, not the pipeline](#what-a-scan-costs) · [that mean is a warm-cache number](#what-a-scan-costs) |
...
|  | cost & latency | 21 logged questions | **≈ $0.003/question** · p50 6.3 s · [1.5× the two-stage pipeline it beat by 1–2 questions](#what-the-extra-stages-cost) · [3% cached, against 65% for a warm scan](#what-a-question-costs-and-what-the-extra-stages-buy) |
...
The 15-clause sample lease: 17 LLM calls + 15 embeddings, ≈ $0.015, ~9 s. Across the 190 scans in the log (9–15 clauses each — evaluation-set builds, demo recordings and verification runs): **mean ≈ $0.0113/scan, p50 9.29 s, p95 15.76 s, max 71.4 s** — latency is dominated by the slowest clause in the pool, not by clause count, which is why the eight-way pool matters more than prompt size.
...
Across 21 logged questions: **mean $0.00301, 5.1 LLM calls, p50 6.3 s** ([`scan_cost_summary.json`](evaluation/scan_cost_summary.json), regenerated by `python -m leasehound.metrics --write`). What the extra stages cost, from `scripts/measure_ask_cost.py` over 12 renter-voice questions:
