...
|  | judge precision | 35 labelled permissive-vs-prohibited clauses | shipped index 8/10 prohibitions flagged · 2 false reds · candidate fix **10/10 flagged · 3 false reds** — [opposite defects, neither shipped](evaluation/README.md#the-labelled-set-said-ship-it-the-40-lease-set-said-no) · [and the judge is not inventing the words, which was the next hypothesis](evaluation/README.md#the-judge-was-not-inventing-anything) |
|  | jurisdiction | 14 labelled leases · 11 real WA documents | 12/14 exact · **0 false alarms on all 11 real leases** · a planted "report this as California" line held — [it never asked until now](evaluation/README.md#jurisdiction--whose-law-is-this-lease-under) |
|  | checklist coverage | all 98 sections of RCW 59.18, against the 5-item checklist | **2 of 5 items fail the project's own admission criterion**, and [1 requirement that meets it is not checked at all](evaluation/README.md#the-checklist-was-never-checked-against-the-statute) |
|  | cost & latency | 201 logged scans (9–15 clauses) | **≈ $0.011/scan** · p50 9.3 s · [p95 15.9 s is the provider, not the pipeline](#what-a-scan-costs) · [that mean is a warm-cache number](#what-a-scan-costs) |
| **ask** | answer quality | 82 generated questions | **82/82 answers consistent with the statute** · 81/82 grounded · 75/82 cite the ground-truth section — [graded against reference statute text](evaluation/README.md#generation-layer-evaluation--is-the-final-answer-right-n82--2-configs), by an LLM judge [no human has yet audited](evaluation/README.md#who-grades-the-grader--a-human-audit-built-and-not-yet-run) |
|  |  | the same 82 questions, same model, no retrieval | **63/82 answers contradict the statute** · 18/82 cite the right section · [one cited section does not exist](evaluation/README.md#closed-book-ask-mode--63-of-82-answers-contradict-the-statute) |
...
|  | jurisdiction | 12 labelled questions · the same 82 questions as controls | 11/12 exact · **12/12 warned exactly when they should have** · [0 false alarms on all 82](evaluation/README.md#jurisdiction--whose-law-is-this-lease-under) |
|  | cost & latency | 35 logged questions | **≈ $0.003/question** · p50 6.5 s · [1.5× the two-stage pipeline it beat by 1–2 questions](#what-the-extra-stages-cost) · [3% cached, against 65% for a warm scan](#what-a-question-costs-and-what-the-extra-stages-buy) |
...
**Measured on both modes, not asserted.** Closed-book, the same model on the same inputs: on the labelled leases it gets [3 of 14 citations right](evaluation/README.md#no-retrieval-baseline--the-same-leases-and-model-closed-book), and on the 82 questions **[63 of 82 answers contradict the statute](evaluation/README.md#closed-book-ask-mode--63-of-82-answers-contradict-the-statute)** — fluently, with citations, one of them to `RCW 59.18.385`, which does not exist.
...
- **Scan mode** — walks your lease clause by clause, retrieves the governing statute for each, and produces a structured red-flag report with citations. Each clause queries the statutes directly, skipping the query-rewriting stages: those bridge renter vocabulary to statute vocabulary (see the adversarial experiment below), and a lease clause already speaks statute — a hybrid lexical channel was [measured here and rejected](evaluation/README.md#hybrid-retrieval-bm25--dense--measured-and-not-shipped) too.
  - *Concurrency.* Clause judgments are independent API calls, so they run on a bounded pool of `MAX_PARALLEL_SCANS = 8` — bounded so a long lease does not trip the provider's rate limits, which also caps what concurrency can buy. A 15-clause lease takes two waves rather than fifteen, so the ceiling is 15/2 = **7.5×**, and at the log's mean of 13 clauses it is 13/2 = 6.5×. This line used to claim "~10 seconds instead of ~90", which is 9× — above the ceiling a reader can compute from a constant in [`scan.py`](leasehound/scan.py). The pool's shape is measured in [`concurrency_results.json`](evaluation/concurrency_results.json): with clause latency stubbed at 4s, 8 clauses finish in one wave of 4.0s and 9 take two waves of 8.0s.
...
The container also **runs as an unprivileged user** now (`hound`, uid 10001), which it did not until the fourth audit went looking. The first code to touch a visitor's upload is `pypdf` parsing an arbitrary PDF, and that was happening as uid 0 — Cloud Run's sandbox is a second line of defence, not a reason to skip the first. Nothing at runtime writes inside `/app`: the vector store is read-only and the report temp files go to `/tmp`.
...
The 15-clause sample lease: 17 LLM calls + 15 embeddings, ≈ $0.015, ~9 s. Across the 201 scans in the log (9–15 clauses each — evaluation-set builds, demo recordings and verification runs): **mean ≈ $0.0114/scan, p50 9.31 s, p95 15.94 s, max 71.4 s**. That band is quoted rather than the whole log because the paragraph after it produces a 49-clause lease as a contrast, and a headline that already contained it would have nothing to contrast with; the whole log — 215 scans, 5 to 49 clauses — is **$0.0117**, a tenth of a cent apart, and both are in [`scan_cost_summary.json`](evaluation/scan_cost_summary.json) — latency is dominated by the slowest clause in the pool, not by clause count, which is why the eight-way pool matters more than prompt size.
...
That exemption is also what made the clause cap a weaker bound than two years of comments here claimed, and the fix runs in the other direction. **A document over 384,000 characters is refused outright**, where one over 60 clauses is merely trimmed. The asymmetry is the whole argument: trimming the clause pass leaves a smaller true report, while skipping a protections window leaves a *false* one, so the only safe way to bound that pass is to bound its input. The check runs before the split and before the gate — no model call, no allocation of the clause list — which makes it the one refusal in this pipeline that costs $0 and the one that `--scan-anyway` cannot override, since the override would be precisely the spend being refused. The limit is ~5.7× the largest real lease measured here and about 128 pages; the arithmetic and the bill it prevents are [under what a scan costs](#what-a-scan-costs).
...
**The same call now answers a second question: whose law governs this document.** `state` was a caller parameter defaulting to `"wa"` and was never read off the lease, so a California renter got a full set of verdicts citing Washington statutes and `Jurisdiction: WA` in the header — a setting printed where a report states its findings. The gate had no opinion because it was asked the wrong question: a California lease *is* a residential lease. It now returns the state the document points to — from a governing-law clause, the premises address, or the statutes it cites — for no extra API call, and a mismatch puts a warning above the verdicts saying **both** directions of what that means: a clause flagged red may be perfectly enforceable where the lease lives, and one marked clear may be void there with nothing in the report to say so. The header says `Judged against: WA law`, which is what it always meant.

`unknown` is not a mismatch, deliberately — most short leases name no state anywhere, and a warning that fires on all of them is one nobody reads. Measured at [12/14 on labelled cases with 0 false alarms across 11 real Washington documents](evaluation/README.md#jurisdiction--whose-law-is-this-lease-under), including a planted line instructing the classifier to report a different state. Refusing on a mismatch would be the wrong trade: the Washington reading of a California lease is still information, and the visitor asked for it.
...
Across 35 logged questions: **mean $0.00288, 4.9 LLM calls, p50 6.45 s** ([`scan_cost_summary.json`](evaluation/scan_cost_summary.json), regenerated by `python -m leasehound.metrics --write`). Every row in it is still tagged `unknown`: the `client` field records which surface asked, and each of these predates it — the summary says so rather than guessing which of them was an eval run. What the extra stages cost, from `scripts/measure_ask_cost.py` over 12 renter-voice questions:
...
<a id="what-the-extra-stages-cost"></a>**So the full pipeline costs 1.5× and adds 1.1 s** for the [+.017 MRR and +5 questions on hit@5](evaluation/README.md#adversarial-rephrasing--the-same-82-questions-renter-voice) that the reworded set gave it over the two-stage pipeline. (**That figure was wrong here until now: it read +.060, which is the full pipeline's own *drop* when the questions are reworded — the smallest drop of any configuration, but a different quantity from a gap over the baseline.** .748 against .731 is +.017; the hit@5 gap, .866 against .805, is the one worth five questions.) Stated plainly: that gain is one or two flipped questions per metric, bought at half again the price. It is kept because $0.0026 and 6.7 s are both far inside what a renter waiting on a legal answer will tolerate, and because the gain pointed the same way on all four metrics — but the honest version of "we keep the full pipeline" includes the price, and until this was measured it didn't.
...
- **Per scan** — the clause cap is 60, so the clause pass is bounded at 60 embeddings + 60 completions. That was never the whole bound: add **one gate call, plus one protections call per 24,000 characters**, because the protections pass is [deliberately exempt from the cap](#when-a-lease-is-longer-than-the-cap) — it reports what the lease omits, so it has to read the part the cap skipped. `60 + 1 + ceil(chars / 24000)` is *bounded in clauses but linear in document length*, and for a while this paragraph said exactly that and stopped there. **A correct description of an unbounded term is not a bound.** Nothing capped the document, and Gradio's upload limit defaults to unlimited, so one 10 MB upload bought 441 windows and ≈ **$1.09** against a measured mean scan of $0.011 — on an endpoint with no auth and no rate limit. Documents are now capped at **384,000 characters** (~128 pages, ~5.7× the largest real lease measured here), which closes the last term at 17 windows and makes the ceiling **`60 + 1 + 17 = 78` completions, flat**. Over the limit a scan is *refused*, not trimmed — unlike the clause cap, and for a reason worth the asymmetry: "clauses 1–60 were judged" is a smaller true report, but a protections window that never ran turns "this lease omits X" from partial into **wrong**. The refusal happens before the split and before the gate, so it is the only one here that costs exactly $0. Measured, from the [format probe](evaluation/README.md#document-formats--real-published-leases-and-real-numbering): the 15-clause sample lease is 17 completions (≈ $0.015), the 49-clause / 40k-character HUD lease is 52 (≈ $0.06), the 270-clause / 68k-character UW agreement is 64 — all far under the ceiling. A *per-scan dollar budget* is still the better instrument ([Roadmap](#roadmap)); this is the bound that stops the bleeding.
- **Per document** — finished scans are cached by content hash, so re-uploading the same lease (or a renamed copy) costs $0.
- **Per instance** — `--max-instances 1` plus a 4-concurrent / 16-queued admission gate, so the fan-out cannot multiply across instances. That gate governs *scans*, and it is worth being precise about what it does not govern: Cloud Run's own `containerConcurrency` on this service is **80**, and an upload arrives over HTTP before any queued handler runs. So uploads are bounded by bytes rather than by the queue — **8 MB**, sized against the 1 GiB instance because Cloud Run has no disk and charges filesystem writes to memory. Enforced on both surfaces from one constant: Gradio's `max_file_size` for the UI, an explicit check on `/v1/scan`, which had none until the size audit went looking.
- **Per question** — the ask path had none of the above, and finding that out is what this section cost. The four bounds here were all about *scans*, because a scan is what reads a file: uploads are capped in bytes and in characters, and a document over the limit is refused before the gate. A question is not a file, so nothing checked it. `/v1/ask` has capped `question` at 2,000 characters since the day it was written — and that is the route which returns 503 on the hosted demo, while the Gradio event handler, which does not, capped nothing. **Gradio publishes its event handlers as HTTP endpoints:** `POST /gradio_api/call/respond` takes `message` and `history` as whatever the caller sends, with no token and no rate limit — verified against the live demo, which answers 200 to a request carrying no token. The one bound in that path, `[-10:]` on the conversation window, counts **messages**; ten unbounded messages are unbounded, and at 400k characters each they are ~1M input tokens — inside both models' 1,047,576-token context, across the router, two query rewrites and the answer. ≈ **$0.70** by litellm's own price map against a measured mean of $0.003, which is arithmetic and not a bill: what is measured here is that the endpoint is open, not that anyone has paid that. The API's own number is now one constant enforced inside `answer_question`, the only code both doors run, and the split is the document limit's: **an oversized question is refused** (answering half a question answers the wrong question) while **an oversized older turn is truncated** (it only costs the model context). This is the third time a bound has turned out to be on one door of two — see also the byte limit that /v1/scan didn't have — and the pattern is worth naming: *the surface that gets audited is the one with a schema.*
...
- Uploads are parsed in memory; the uploaded temp file is deleted immediately after parsing, and lease text never enters the vector store. The per-scan metrics log records counts and cost. It used to record the file name too, and this bullet used to offer that as the reassuring half of the sentence — but a lease file name is a person (`SmithJohn_Lease.pdf`, `1425_Pine_St_Apt_5B.pdf` is an address), the log line is also printed to stdout, and on Cloud Run stdout is Cloud Logging. So the demo was accumulating strangers' names in the operator's own project to serve a need that only exists on a laptop: matching a log row to a document you still have. **The name is now kept only for the `cli` and `eval` clients; `ui` and `api` log the extension alone** (`upload.pdf`), which is the part that is about the document rather than about its owner.
- Scan results are cached in process memory only (keyed by content hash, bounded, never written to disk) and vanish when the instance scales to zero. One thing *is* written to disk, and this bullet used to read as though nothing were: the **downloadable report**, because a download button can only serve a file, and the report quotes the clauses it judged. It goes to a temp directory that is now bounded to the 32 most recent and pruned on every write — it previously created a fresh directory per scan and deleted none of them, which on Cloud Run is not a disk but a slice of the same 1 GiB the scan is running in.
...
```bash
pip install -e ".[dev]"
pytest          # 239 test functions across 22 files — see below for what they cover
...
- **The judge over-reads accurate text, which is not the defect this line used to name** — it said the judge was *inventing* a word the clause does not contain, and [that was tested and is false](evaluation/README.md#the-judge-was-not-inventing-anything): asked to quote the words each verdict turned on, it produced **23 verbatim quotes out of 23 red verdicts** and then drew "electronic payment only" out of *"by check or electronic transfer as directed by Landlord"* — real words, wrong inference, and nothing a code check can catch. The quote judge cost precision (2 → 4 false reds on the labelled set) and was not shipped. What remains open is a *pair*: that judge together with the rejected enumerated split cleared the gold set with **0 false reds** — the gate the split failed in July — and scored **10/10 prohibitions with 2 false reds**, beating the live configuration on both axes. It needs a rebuilt runtime index and a **$0.6** re-run of the 40-lease set, and its two remaining false reds are the same terse-payment-clause family that blocked the split before, so the cheap set predicts the expensive one would fail again.
- **Fix the protections checklist against its own criterion** — the [coverage audit](evaluation/README.md#the-checklist-was-never-checked-against-the-statute) found two of five shipped items satisfiable by a posted notice or a separate letter rather than by lease text (mold, deposit location), and one requirement that *is* lease text and is not checked: RCW 59.18.285, under which an undesignated "nonrefundable" fee **is refundable**. All three are one change, because `protections_exact` is an exact set match and any of them means re-labelling all 46 labelled leases plus a paid re-run of both sets.
- **Merge short clauses instead of dropping them** — clause splitting discards any numbered clause under 80 characters, which is the right treatment for a heading (`3. ELIGIBILITY`) and the wrong one for a provision (`11. WAIVER. Tenant waives all rights under RCW 59.18.`). Nothing in the labelled sets is affected — **0 clauses dropped across all 46 of them** — but two real university housing agreements lose 61 and 28 fragments each, mostly headings and not entirely. Found by a [jurisdiction case](evaluation/README.md#jurisdiction--whose-law-is-this-lease-under) whose premises address sat in a 66-character clause and never reached the gate.
...
- Replace the 60-clause cap with a per-scan dollar budget — the cap now [degrades instead of refusing](#when-a-lease-is-longer-than-the-cap), but a clause count is a proxy for spend, and clauses vary in length by 10×. A budget would let a 270-clause agreement of short provisions finish where a cap of 60 stops it arbitrarily. It would also subsume the [384k-character document cap](#what-a-scan-costs), which bounds the protections pass by bounding its input — the right fix for an unbounded bill, and a blunt one for a document that is merely long
