...
The 15-clause sample lease: 17 LLM calls + 15 embeddings, ≈ $0.015, ~9 s. Across the 201 scans in the log (9–15 clauses each — evaluation-set builds, demo recordings and verification runs): **mean ≈ $0.0114/scan, p50 9.31 s, p95 15.94 s, max 71.4 s** — latency is dominated by the slowest clause in the pool, not by clause count, which is why the eight-way pool matters more than prompt size.
